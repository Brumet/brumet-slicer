...
const DEFAULT_CFG = {
  ...
  marca: {
    nombre:    'Brumet',
    subtitulo: 'Cotizador de Impresión 3D · Bogotá, Colombia',
    contacto:  '@brumet.co · 310 607 8712',
    color:     '#E0313A',
    logo:      ''   // dataURL de la imagen del logo (vacío = logo Brumet)
  }
}
...
ipcMain.handle('load-settings', () => {
  try {
    if (fs.existsSync(SETTINGS_FILE)) {
      ...
      return { ...DEFAULT_CFG, ...saved, marca: { ...DEFAULT_CFG.marca, ...(saved.marca || {}) } }
    }
  } catch(e) {}
  return DEFAULT_CFG
})
...
ipcMain.handle('export-pdf', async (event, data) => {
  ...
  const marcaSlug = ((data.marca && data.marca.nombre) || 'Brumet').replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ0-9]/g, '_').slice(0, 30)
  const { filePath } = await dialog.showSaveDialog(mainWin, {
    title: 'Guardar cotización PDF',
    defaultPath: `Cotizacion_${marcaSlug}${clientSlug}_${new Date().toISOString().slice(0, 10)}.pdf`,
...
.hdr{display:flex;justify-content:space-between;align-items:center;padding-bottom:18px;border-bottom:3px solid ${color};margin-bottom:24px}
...
h2{font-size:9px;font-weight:800;color:${color};text-transform:uppercase;letter-spacing:.18em;margin:20px 0 9px;padding-bottom:5px;border-bottom:1px solid #eee}
...
.price-box{margin-top:24px;background:${color};border-radius:12px;padding:24px;text-align:center;color:#fff}
...
<div class="hdr">
  <div>${logoHTML}<div class="brand-sub">${marca.subtitulo}</div></div>
  <div class="doc-right">
...
<div class="footer">${marca.nombre}${marca.contacto ? ' · ' + marca.contacto : ''} · Generado con Brumet Slicer v${app.getVersion()}</div>
</body></html>`
}
